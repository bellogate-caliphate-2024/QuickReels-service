...
import {
  ...
  import {
    firebaseAdmin,
    firebaseApp,
  } from '../data/Abstarcts/DataBase/Firebase';
  ...
  export class PostsService {
    ...
    async createPost(
      ...
      try {
        ...
        if (!createUserDto.video_url || !Array.isArray(createUserDto.video_url)) {
          createUserDto.video_url = [];
        }
        if (!createUserDto.thumbnail || !Array.isArray(createUserDto.thumbnail)) {
          createUserDto.thumbnail = [];
        }
        // Add video URL and thumbnail URL to DTO
        createUserDto.video_url.push(videoDownloadURL);
        createUserDto.thumbnail.push(thumbnailDownloadURL);
        createUserDto.time = new Date().toISOString();
